-- group users by week 
SELECT id, week(sale_date) AS sale_week
FROM sale
GROUP BY id, week(sale_date);

-- extract first week of purchase for each user 
SELECT id, min(week(sale_date)) AS start_week
FROM sale
GROUP BY id;

-- merge the tables above 
SELECT a.id, a.sale_date, b.start_week AS start_week
FROM
(
SELECT id, week(sale_date) AS sale_date
FROM sale
...
SELECT first_week,
     SUM(CASE WHEN week_num = 0 THEN 1 ELSE 0 END) AS week_0,
       ...
       FROM  (    
       SELECT a.id,a.sale_date,b.start_week,a.sale_date-start_week as week_num  
       FROM   
       (SELECT id, sale_date
        FROM sale
		GROUP BY id,week(sale_date)) a,
